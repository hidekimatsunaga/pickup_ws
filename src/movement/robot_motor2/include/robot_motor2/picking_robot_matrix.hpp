...
private:
    ...
    const float wheel_dist[size] = {0.32330, 0.32330, 0.38500}; // ロボット中心から各車輪までの距離
    const float wheel_phi[size] = {-0.883978, 0.883978, M_PI};  // ロボット中心から見た各車輪の位置(極座標)
...
PickingRobotMatrix::~PickingRobotMatrix()
{
    std::cout << "Close the BlowerRobotMatrix" << std::endl;
...
void PickingRobotMatrix::calcWheelVelAng(const double &vx, const double &vy, const double &vth, double *drive_vel, float *steer_phi)
{
    ...
    V(2, 0) = vth;
    V_wheel = R * V;
    // RCLCPP_INFO(this->get_logger(), "Input velocities: vx=%f, vy=%f, vth=%f", vx, vy, vth);

    // 逆運動学計算
    for (int i = 0; i < size; i++)
    {
        // RCLCPP_INFO(this->get_logger(), "V_wheel[%d]: x=%f, y=%f", i, V_wheel(i * 2, 0), V_wheel(i * 2 + 1, 0));
        ...
        // RCLCPP_INFO(this->get_logger(), "Calculated drive_vel[%d]: %f, steer_phi[%d]: %f", i, drive_vel[i], i, steer_phi[i]);
