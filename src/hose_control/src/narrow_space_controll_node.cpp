...
public:
    ...
    sequence_step_(0) //シーケンスの現在のステップ
  {
    ...
    current_angles_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      ...
      std::bind(&NarrowSpaceControllNode::currentAnglesCallback, this, std::placeholders::_1)
    );
    
    sub_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
      "/hose/goal_point", 10,
      std::bind(&NarrowSpaceControllNode::callback, this, std::placeholders::_1)
    );

    air_sub_ = create_subscription<std_msgs::msg::Float32>(
      "/sensor/pressure", 10,
      std::bind(&NarrowSpaceControllNode::airCallback, this, std::placeholders::_1));
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<NarrowSpaceControllNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
